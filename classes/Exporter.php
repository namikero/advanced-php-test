...
class Exporter {
    ...
    function getPlayers($search) {
        $where = [];
        if ($search->has('playerId')) $where[] = "id = '" . $search['playerId'] . "'";
        if ($search->has('player')) $where[] = "name = '" . $search['player'] . "'";
        if ($search->has('team')) $where[] = "team_code = '" . $search['team']. "'";
        if ($search->has('position')) $where[] = "position = '" . $search['position'] . "'";
        if ($search->has('country')) $where[] = "nationality = '" . $search['country'] . "'";
        $where = implode(' AND ', $where);
        $sql = "
            SELECT roster.*
            FROM roster
